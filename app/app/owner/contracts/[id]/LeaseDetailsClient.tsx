...
// Config des statuts - FLUX COMPLET
const STATUS_CONFIG: Record<string, { label: string; color: string; description?: string }> = {
  draft: { label: "Brouillon", color: "bg-slate-100 text-slate-700 border-slate-300", description: "Le bail est en cours de rédaction" },
  sent: { label: "Envoyé", color: "bg-blue-100 text-blue-700 border-blue-300", description: "Le bail a été envoyé pour signature" },
  pending_signature: { label: "Signature en attente", color: "bg-amber-100 text-amber-700 border-amber-300", description: "En attente de toutes les signatures" },
  partially_signed: { label: "Partiellement signé", color: "bg-orange-100 text-orange-700 border-orange-300", description: "Certaines parties ont signé" },
  pending_owner_signature: { label: "À signer (propriétaire)", color: "bg-blue-100 text-blue-700 border-blue-300", description: "Attente de la signature du propriétaire" },
  fully_signed: { label: "✅ Signé - Prêt pour EDL", color: "bg-indigo-100 text-indigo-700 border-indigo-300", description: "Signé, en attente de l'état des lieux d'entrée" },
  active: { label: "Actif", color: "bg-green-100 text-green-700 border-green-300", description: "Le bail est en cours" },
  amended: { label: "Avenant", color: "bg-purple-100 text-purple-700 border-purple-300", description: "Avenant en cours" },
  terminated: { label: "Terminé", color: "bg-slate-100 text-slate-600 border-slate-300", description: "Le bail est terminé" },
  archived: { label: "Archivé", color: "bg-gray-200 text-gray-600 border-gray-300", description: "Le bail est archivé" },
};
                    ...
                    <div className="p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <Button variant="link" size="sm" className="h-auto p-0 text-red-600 text-xs font-bold" asChild>
                          <Link href={`/app/owner/properties/${property.id}/diagnostics`}>
                ...
                {dpeStatus?.status !== "VALID" && (
                  <div className="p-3 bg-amber-50 border-b border-amber-100 flex items-center justify-between group">
                    ...
                    <Button variant="ghost" size="sm" className="h-7 text-[10px] bg-white border-amber-200 hover:bg-amber-100" asChild>
                      <Link href={`/app/owner/properties/${property.id}/diagnostics`}>Régulariser</Link>
                    </Button>
                  </div>
                )}
            ...
            <Card className="border-none shadow-sm bg-white">
              ...
              <CardContent className="pt-2 p-2">
                <nav className="space-y-1">
                  ...
                  <Link 
                    href={`/app/owner/inspections?lease_id=${leaseId}`}
                    className="flex items-center justify-between px-3 py-2 text-sm font-medium rounded-md hover:bg-slate-50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
